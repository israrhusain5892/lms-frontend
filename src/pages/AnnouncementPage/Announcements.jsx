...
const Announcements = ({ announcement }) => {
    const renderStars = (rating) => {
        return Array.from({ length: 5 }).map((_, index) => {
            if (index < Math.floor(rating)) {
                return <IoMdStar color="#FCD34D" key={index} />;
            } else if (index < rating) {
                return <IoMdStarHalf color="#FCD34D" key={index} />;
            } else {
                return <IoMdStarOutline color="#FCD34D" key={index} />;
            }
        });
    };
        ...
        <div className="announcement-card">
            <div className="announcement-header">
                ...
                <div className="announcement-info">
                    <h5 className="announcement-name">{announcement.name}</h5>
                    <small className="announcement-meta">
                        <span>{announcement.username}</span> <span>{announcement.date}</span>
                    </small>
                    <div className="announcement-stars">{renderStars(announcement.rating)}</div>
                </div>
